...
You can test a few different things. The `/hello` endpoint only sends the question to the LLM, and the `/rag` endpoint goes to Neo4j before forwarding the returned graph data to the LLM for formatting the response. Here are some ideas for values to test the `/rag` endpoint:
