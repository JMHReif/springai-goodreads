...
You can test a few different things. The `/llm` endpoint only sends the question to the LLM, and the `/rag` endpoint goes to Neo4j before forwarding the returned graph data to the LLM for formatting the response. Here are some ideas for values to test the `/rag` endpoint:

```
http ":8080/rag?searchPhrase=fairytale%20ending"

http ":8080/rag?searchPhrase=jedi”

http ":8080/rag?searchPhrase=dragons”
```
